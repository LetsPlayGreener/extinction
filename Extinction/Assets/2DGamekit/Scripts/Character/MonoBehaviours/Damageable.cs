        ...
        public void TakeDamage(Damager damager, bool ignoreInvincible = false)
        {
            ...
            if (LearningAnalyticsGenerator.instance && LearningAnalyticsGenerator.instance.canGenerateLA)
            {
                if(gameObject.layer == 16)
                    GBL_Interface.SendStatement("damaged", "destructible", "DestructableElement", new Dictionary<string, List<string>>()
                    {
                        { "value", new List<string>() { m_CurrentHealth.ToString() } }
                    });
            }
            ...
            if (m_CurrentHealth <= 0)
            {
                ...
                {
                    if (gameObject.layer == 16)
                        GBL_Interface.SendStatement("destroyed", "destructible", "DestructableElement");
                }
            }
        }
