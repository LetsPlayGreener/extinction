    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        public void OnHurt(Damager damager, Damageable damageable)
        {
            ...
            if (LearningAnalyticsGenerator.instance && LearningAnalyticsGenerator.instance.canGenerateLA)
                GBL_Interface.SendStatement("received", "damage", "Damage", new Dictionary<string, List<string>>()
                    {
                        { "from", new List<string>() { damager.gameObject.name } }
                    });
        ...
        public void Respawn(bool resetHealth, bool useCheckpoint)
        {
            ...
            if (useCheckpoint && m_LastCheckpoint != null)
            {
                ...
                if (LearningAnalyticsGenerator.instance && LearningAnalyticsGenerator.instance.canGenerateLA)
                    GBL_Interface.SendStatement("respawned", "avatar", "Alex", new Dictionary<string, List<string>>()
                    {
                        { "position", new List<string>() { m_LastCheckpoint.transform.position.ToString() } }
                    });
            }
            else
            {
                ...
                if (LearningAnalyticsGenerator.instance && LearningAnalyticsGenerator.instance.canGenerateLA)
                    GBL_Interface.SendStatement("respawned", "avatar", "Alex", new Dictionary<string, List<string>>()
                    {
                        { "position", new List<string>() { new Vector3(m_StartingPosition.x, m_StartingPosition.y, 0).ToString() } }
                    });
            }
        }
